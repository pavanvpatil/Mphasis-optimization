...
def initGUI() -> None:
    ...
    layout.append([psg.Checkbox("Premium Economy Class", key="premium_eco_class", default=True, enable_events=True, size=(30, 1)),
    ...
    layout.append([psg.Checkbox("Arrival Time Score Multiplier", default=True, key='arr_time_multiplier_check', size=(30, 1), enable_events=True),
                   psg.Input("70", key='arr_time_multiplier', enable_events=True)])

    layout.append([psg.Checkbox("Departure Time Score Multiplier", default=True, key='dep_time_multiplier_check', size=(30, 1), enable_events=True),
                   psg.Input("70", key='dep_time_multiplier', enable_events=True)])

    layout.append([psg.Checkbox("Equipment Score Multiplier", default=True, key='equipment_score_check', size=(30, 1), enable_events=True),
    ...
    layout.append([psg.Text("Number of top flights considered", size=(
        32, 1), enable_events=True), psg.Input("3", key='no_of_top_flights', enable_events=True)])

    layout.append([psg.Text("Minimum connection time in hours", size=(
        32, 1), enable_events=True), psg.Input("1", key='min_connection_time_hrs', enable_events=True)])

    layout.append([psg.Text("Maximum connection time in hours", size=(
        32, 1), enable_events=True), psg.Input("12", key='max_connection_time_hrs', enable_events=True)])

    layout.append([psg.Button("SAVE", key='save', size=(None, 1), pad=10)])

    window = psg.Window("Mphasis Optimization Tool",
                        element_justification="center", layout=layout, resizable=True)
    ...
    while True:
        ...
        if event == psg.WIN_CLOSED or event == "save":
            ...
            if flight_ranking_values_obj.max_connection_time_hrs < flight_ranking_values_obj.min_connection_time_hrs:
                psg.popup(
                    "Maximum connection time cannot be less than minimum connection time")
                continue

            if flight_ranking_values_obj.max_connection_time_hrs < 0 or flight_ranking_values_obj.min_connection_time_hrs < 0:
                psg.popup(
                    "Connection time cannot be negative")
                continue

            if flight_ranking_values_obj.max_dowline_flights <= 0:
                psg.popup(
                    "Maximum downline flights must be greater than 0")
                continue

            if flight_ranking_values_obj.no_of_top_flights <= 0:
                psg.popup(
                    "Number of top flights must be greater than 0")
                continue

            if flight_ranking_values_obj.distance_score < 0:
                psg.popup(
                    "Distance score multiplier cannot be negative")
                continue

            if flight_ranking_values_obj.arr_time_multiplier < 0:
                psg.popup(
                    "Arrival time score multiplier cannot be negative")
                continue

            if flight_ranking_values_obj.dep_time_multiplier < 0:
                psg.popup(
                    "Departure time score multiplier cannot be negative")
                continue

            if flight_ranking_values_obj.equipment_multiplier < 0:
                psg.popup(
                    "Equipment score multiplier cannot be negative")
                continue

            if flight_ranking_values_obj.stop_over_score > 0:
                psg.popup(
                    "Stop over score cannot be positive")
                continue

            if pnr_ranking_values_obj.loyalties_pplatinum < 0 or pnr_ranking_values_obj.loyalties_platinum < 0 or pnr_ranking_values_obj.loyalties_gold < 0 or pnr_ranking_values_obj.loyalties_silver < 0 or pnr_ranking_values_obj.ssr_score < 0 or pnr_ranking_values_obj.pax_cnt_score < 0 or pnr_ranking_values_obj.first_class_score < 0 or pnr_ranking_values_obj.business_class_score < 0 or pnr_ranking_values_obj.eco_class_score < 0 or pnr_ranking_values_obj.premium_eco_class_score < 0:
                psg.popup(
                    "PNR ranking values cannot be negative")
                continue
